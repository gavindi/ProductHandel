...
class Product_Handel_Post_Payment {
    ...
    public function handle($order_id, $ipn_data) {
        ...
        if ($generate_key) {
            ...
            if ($salt) {
                $license_key = Product_Handel_Order_Manager::generate_license_key($order->buyer_first_name, $order->buyer_last_name, $order->buyer_email, $salt, $order->transaction_id);
